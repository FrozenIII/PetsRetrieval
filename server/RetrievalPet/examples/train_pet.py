...
from RetrievalPet.layers.loss import ContrastiveLoss
from RetrievalPet.datasets.datahelpers import collate_tuples, cid2filename
from RetrievalPet.datasets.traindataset_pet import TuplesDataset
...
training_dataset_names = ['pet']
test_datasets_names = ['pet']
...
parser.add_argument('--training-dataset', '-d', metavar='DATASET', default='pet',
                    choices=training_dataset_names,
                    help='training dataset: ' +
                         ' | '.join(training_dataset_names) +
                         ' (default: retrieval-SfM-120k)')
parser.add_argument('--no-val', dest='val', action='store_true',
...
parser.add_argument('--test-datasets', '-td', metavar='DATASETS', default='pet',
...
parser.add_argument('--query-size', '-qs', default=4000, type=int, metavar='N',
...
#     download_test(get_data_root())
    ...
    directory += "use_caption{}/".format(str(args.use_caption))
    ...
    model_params['use_caption'] = args.use_caption
    ...
    start_epoch = 0
    if args.resume:
        args.resume = os.path.join('/root/server/EXPORT_DIR=petModel/pet_resnet101_gem_softmax_m0.70/_'
                                   'adam_lr1.0e-06_wd1.0e-04_gid1/'
                                    '_nnum5_qsize17645_psize2000/_bsize16_imsize1024/', args.resume)
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        for q in range(nq):
            ...
            for imi in range(ni):
                ...
                if args.use_caption:
                    output[:, imi] = model(input[q][imi][0].cuda(), torch.from_numpy(input[q][imi][1]).cuda(), True).squeeze()
                else:
                    output[:, imi] = model(input[q][imi][0].cuda()).squeeze()
...
def test(datasets, net):
    ...
    if args.test_whiten:
        ...
        wvecs = extract_vectors(net, images, image_size, transform,print_freq=10,batchsize=2000)  # implemented with torch.no_grad
    ...
    for dataset in datasets:
        ...
        if len(ms) > 1 and net.meta['pooling'] == 'gem' and not net.meta['regional'] and not net.meta['whitening']:
            ...
            msp = 1
        print('>> {}: dataset images...'.format(dataset))
        vecs = extract_vectors(net, images, image_size, transform, bbxs, ms=ms, msp=msp, print_freq=1000,batchsize=20,use_cap=images_cap)  # implemented with torch.no_grad
        print('>> {}: query images...'.format(dataset))
        qvecs = extract_vectors(net, qimages, image_size, transform, bbxs, ms=ms, msp=msp,  print_freq=1000,batchsize=20,use_cap=qimages_cap)  # implemented with torch.no_grad
