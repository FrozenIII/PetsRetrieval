...
from RetrievalPet.layers.pooling import MAC, SPoC, GeM, RMAC, Rpool, HNIP
from RetrievalPet.layers.normalization import L2N, PowerLaw
...
class ImageRetrievalNetCaption(nn.Module):
    def __init__(self, features, lwhiten, pool, whiten, meta, poolname):
        super(ImageRetrievalNetCaption, self).__init__()
        self.features = nn.Sequential(*features)
        self.lwhiten = lwhiten
        self.pool = pool
        self.whiten = whiten
        ...
        self.meta = meta
        self.poolname = poolname
        self.linear1 = nn.Linear(meta['outputdim']+600,meta['outputdim']+600, bias=False)
        self.linear2 = nn.Linear(meta['outputdim'] + 600, meta['outputdim'], bias=False)
    def forward(self, x,y,useY=False):
        # x -> features
        o = self.features(x)
        # TODO: properly test (with pre-l2norm and/or post-l2norm)
        # if lwhiten exist: features -> local whiten
        if self.lwhiten is not None:
            ...
            o = o.view(s[0], s[2], s[3], self.lwhiten.out_features).permute(0, 3, 1, 2)
            # o = self.norm(o)
        #batch,dim,1,1 after pool
        o = self.pool(o)
        o = o.squeeze(-1).squeeze(-1)
        if useY:
            caption = y.unsqueeze(0)
            # print(o.size(),caption.size())
            o = torch.cat((o, caption), dim=1)
            o = self.linear1(o)
            o = self.linear2(o)
        o = self.norm(o)
        # if whiten exist: pooled features -> whiten -> norm
...
def init_network(params):
    ...
    use_caption = params.get('use_caption', False)
    ...
    print()
    if use_caption:
        net = ImageRetrievalNetCaption(features, lwhiten, pool, whiten, meta, poolname=pooling)
    else:
...
def extract_vectors(net, images, image_size, transform, bbxs=None, ms=[1], msp=1, print_freq=10, batchsize=300, isbase64=False, noneedTcuda=False,use_cap=None):
    ...
    loader = torch.utils.data.DataLoader(
        ImagesFromList(root='', images=images, imsize=image_size, bbxs=bbxs, transform=transform, isbase64=isbase64),
        ...
        with torch.no_grad():
            ...
            for i, input in enumerate(loader):
                ...
                if len(ms) == 1:
                    if batchsize!=1:
                        if (i+1) != len(loader):
                            vecs[:, i*batchsize:(i+1)*batchsize] = extract_ss(net, input,use_cap=use_cap[i*batchsize:(i+1)*batchsize])
                        else:
                            vecs[:, i * batchsize:len(images)] = extract_ss(net, input,use_cap=use_cap[i * batchsize:len(images)])
                    else:
                        vecs[:, i] =extract_ss(net, input,use_cap=use_cap[i])
                else:
                    if batchsize!=1:
                        if (i+1) != len(loader):
                            vecs[:, i*batchsize:(i+1)*batchsize] = extract_ms(net, input, ms, msp, batchsize,use_cap=use_cap[i*batchsize:(i+1)*batchsize])
                        else:
                            vecs[:, i * batchsize:len(images)] = extract_ms(net, input, ms, msp, len(images)-i*batchsize, morebatch=True,use_cap=use_cap[i * batchsize:len(images)])
                    else:
                        vecs[:, i] =extract_ms(net, input, ms, msp, batchsize,use_cap=use_cap[i])
        ...
        with torch.no_grad():
            ...
            for i, input in enumerate(loader):
                ...
                if len(ms) == 1:
                    if batchsize != 1:
                        if (i + 1) != len(loader):
                            vecs[:, i * batchsize:(i + 1) * batchsize] = extract_ss(net, input, use_cap=use_cap)
                        else:
                            vecs[:, i * batchsize:len(images)] = extract_ss(net, input,
                                                                            use_cap=use_cap)
                    else:
                        vecs[:, i] = extract_ss(net, input, use_cap=use_cap)
                else:
                    if batchsize != 1:
                        if (i + 1) != len(loader):
                            ...
                            vecs[:, i * batchsize:len(images)] = extract_ms(net, input, ms, msp,
                                                                            len(images) - i * batchsize, morebatch=True,
                                                                            use_cap=use_cap)
                    else:
                        vecs[:, i] = extract_ms(net, input, ms, msp, batchsize, use_cap=use_cap)
...
def extract_ss(net, input,use_cap=None):
    if use_cap.any()!=None:
        g = torch.from_numpy(np.asarray(use_cap)).cuda()
        return net(input,g).cpu().data.squeeze()
    else:
        return net(input,use_cap).cpu().data.squeeze()

def extract_ms(net, input, ms, msp, batchsize, morebatch=False,use_cap=None):
    ...
    for s in ms: 
        ...
        if use_cap.any() != None:
            g = torch.from_numpy(np.asarray(use_cap)).cuda()
            v += net(input_t,g).pow(msp).cpu().data.squeeze()
        else:
