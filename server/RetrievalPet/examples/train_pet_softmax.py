...
from RetrievalPet.datasets.traindataset_pet_softmax import TuplesDataset
...
parser.add_argument('--loss', '-l', metavar='LOSS', default='softmax',
                    choices=loss_names,
...
def train(train_loader, model, criterion, optimizer, epoch, temp_loss):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        output = torch.zeros(nq,model.meta['outputdim']).cuda()
        for q in range(nq):
            output[q,:] = model(input[q].cuda()).squeeze()
...
def test(datasets, net):
    ...
    for dataset in datasets:
        ...
        if len(ms) > 1 and net.meta['pooling'] == 'gem' and not net.meta['regional'] and not net.meta['whitening']:
            ...
            msp = 1
        vecs = extract_vectors(net, images, image_size, transform, bbxs, ms=ms, msp=msp, print_freq=1000,batchsize=20)  # implemented with torch.no_grad
        print('>> {}: query images...'.format(dataset))
        qvecs = extract_vectors(net, qimages, image_size, transform, bbxs, ms=ms, msp=msp,  print_freq=1000,batchsize=20)  # implemented with torch.no_grad
